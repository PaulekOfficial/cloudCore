    ...
    public boolean execute(CommandSender sender, String[] args) {
        if(args.length != 0){
            ...
            if(MsgStorage.getMessages().containsKey(uuid)) {
                ...
                if(Bukkit.getPlayer(MsgStorage.getMessages().get(uuid)) != null) {
                    Bukkit.getPlayer(MsgStorage.getMessages().get(uuid)).sendMessage(message.replace("{from}", sender.getName()));
                    sender.sendMessage(message.replace("{from}", sender.getName()));
                    for(Player p : Bukkit.getOnlinePlayers()){
                        UUID u = p.getUniqueId();
                        if(UserStorage.getUser(u).getSettings().isSocialspy()){
                            String m = Util.fixColor(Lang.INFO_MSG_SPYFORMAT.replace("{from}", sender.getName()).replace("{to}", Bukkit.getPlayer(MsgStorage.getMessages().get(uuid)).getDisplayName()).replace("{message}", s));
                            p.sendMessage(m);
                        }
                    }
                }  else {
