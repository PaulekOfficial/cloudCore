    ...
    public boolean execute(CommandSender sender, String[] args) {
        if(args.length != 0){
            ...
            if(PrivateMessagesStorage.getMessages().containsKey(uuid)) {
                ...
                if(Bukkit.getPlayer(PrivateMessagesStorage.getMessages().get(uuid)) != null) {
                    Bukkit.getPlayer(PrivateMessagesStorage.getMessages().get(uuid)).sendMessage(message.replace("{from}", sender.getName()));
                    sender.sendMessage(message.replace("{from}", sender.getName()));
                    for(Player p : Bukkit.getOnlinePlayers()){
                        UUID u = p.getUniqueId();
                        if(UserStorage.getUser(u).getSettings().isSocialspy()){
                            String m = Util.fixColor(Lang.INFO_MSG_SPYFORMAT.replace("{from}", sender.getName()).replace("{to}", Bukkit.getPlayer(PrivateMessagesStorage.getMessages().get(uuid)).getDisplayName()).replace("{message}", s));
                            p.sendMessage(m);
                        }
                    }
                }  else {
